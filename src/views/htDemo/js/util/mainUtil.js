...
const mainUtil = {
  ...
  getCabinetChilds(cabinetNode, homeDm) {
    ...
    if (homeDm) {
      result.childsArr.push(homeDm.getDataByTag('serverPanel'))
      result.childsArr.push(homeDm.getDataByTag('coreSwitchPanel'))
    }
    if (childs.length > 0) {
      for (let i = 0, l = childs.length; i < l; i++) {
        ...
        } else if (/弹窗/.test(displayName)) {
          result.childsObj['tip'] = temp
          result.childsArr.push(temp)
        } else if (/显卡/.test(displayName)) {
          !result.childsObj['blinkLight'] && (result.childsObj['blinkLight'] = [])
          !result.childsObj['card'] && (result.childsObj['card'] = [])
          const childs = temp.getChildren()
          childs.forEach(child => {
            const childDisplayName = child.getDisplayName()
            if (childDisplayName && childDisplayName.indexOf('闪灯') > -1) {
              result.childsObj['blinkLight'].push(child)
            }
